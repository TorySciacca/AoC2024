...
function day9(dataInput){
  ...
  console.log('sdm: ',spacelessDiskMap)
  ...
  for (let i = 0; i < diskMapData.length; i += 1) {
    ...
    if (diskMapData[i] !== ".") {
      condensedDataMap += diskMapData[i];
    } else {
      condensedDataMap += spacelessDiskMap.slice(-1);
      spacelessDiskMap = spacelessDiskMap.slice(0, -1);
    }

    if (condensedDataMap.length + amountOfSpaces === diskMapData.length) { //note: spacelessDiskMap is now empty
      condensedDataMap += ".".repeat(amountOfSpaces);
      break;
    }
    console.log(condensedDataMap)
  }
  ...
  for (let i = 0; i < condensedDataMap.length; i += 1) {
    if (condensedDataMap[i] !== ".") {
      sum += (i * parseInt(condensedDataMap[i]))
    } else {
